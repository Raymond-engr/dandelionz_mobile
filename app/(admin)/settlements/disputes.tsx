...
export default function AdminDisputesManagement() {
  ...
  const { data: response, isLoading, refetch } = useGetAllDisputesQuery({ status: activeTab });
    ...
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      <View className="flex-row px-[21px] py-4 gap-2">
        {(['PENDING', 'APPROVED', 'REJECTED'] as const).map((status) => (
          ...
          >
            <Text className={`text-[12px] font-bold ${activeTab === status ? "text-white" : "text-[#6B7280]"}`}>
      ...
      <FlatList
            ...
            <View className="p-[21px]">
              <View className="flex-row justify-between mb-2">
                <View className="flex-1">
                  <Text className="text-[14px] font-bold text-system-blue-dark">Order: {item.order_id}</Text>
                  <Text className="text-[12px] text-gray-500 mt-1">Customer: {item.customer_name}</Text>
                  <Text className="text-[12px] text-gray-500">Vendor: {item.vendor_name}</Text>
                </View>
              ...
              <View className="mt-3 p-3 bg-gray-50 rounded-lg">
                <Text className="text-[12px] text-gray-600 italic">"{item.reason}"</Text>
              </View>
              ...
              {activeTab === 'PENDING' && (
                <View className="flex-row gap-3 mt-4">
                  ...
                  >
                    <Text className="text-white text-[13px] font-bold">Approve</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    onPress={() => handleResolve(item.id, 'REJECT')}
                    className="flex-1 bg-red-600 py-3 rounded-xl items-center"
                  >
                    <Text className="text-white text-[13px] font-bold">Reject</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
        ...
        ListEmptyComponent={() => (
          <View className="items-center justify-center py-20 px-10">
            ...
            <Text className="text-gray-400 mt-4 text-center">No {activeTab.toLowerCase()} disputes found.</Text>
          </View>
        )}
      />
