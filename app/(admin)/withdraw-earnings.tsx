...
import { useGetWalletStatsQuery, useAdminRequestWithdrawalMutation } from "@/lib/api/adminApi";
...
export default function AdminWithdrawEarnings() {
  ...
  const [requestWithdrawal, { isLoading: isRequesting }] = useAdminRequestWithdrawalMutation();
    ...
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      <ScrollView className="flex-1 px-[21px] pt-6" showsVerticalScrollIndicator={false}>
        <View className="bg-system-blue-light rounded-[16px] p-8 mb-8 shadow-lg shadow-blue-900/20">
          <Text className="text-white/80 text-[14px] font-medium mb-2 uppercase tracking-widest text-center">Available for Withdrawal</Text>
        ...
        <View className="mb-8">
          <Text className="text-[14px] font-semibold text-system-blue-dark mb-4 text-center">Amount to Withdraw</Text>
        ...
        <View className="mb-10 items-center">
          <Text className="text-[14px] font-semibold text-system-blue-dark mb-4">Payment PIN</Text>
          <PinInput value={pin} onChange={setPin} />
          <Text className="text-[12px] text-gray-400 mt-4 text-center">Enter your 4-digit secure payment PIN</Text>
        </View>
        ...
        <Pressable className="mt-6 py-2" onPress={() => router.push("/(admin)/payment-settings/forgot-pin" as any)}>
          <Text className="text-system-blue-light text-center font-semibold">Forgot PIN?</Text>
        </Pressable>
      </ScrollView>
    </View>
  );
