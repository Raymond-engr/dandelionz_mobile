      ...
      <View className="p-4">
        ...
        <View className="flex-row gap-3 mb-6">
          <View className="flex-1 bg-[rgba(77,255,151,0.25)] rounded-[12px] p-4">
            <Text className="text-[14px] text-[#207d47] font-medium mb-1">
              Active Vendors
            </Text>
            {isLoading ? (
              <View className="h-8 w-12 bg-green-200/50 rounded" />
            ) : (
              <Text className="text-[24px] font-bold text-[#000011]">
                {activeVendors}
              </Text>
            )}
          </View>
          <View className="flex-1 bg-[rgba(255,77,77,0.25)] rounded-[12px] p-4">
            <Text className="text-[14px] text-[#760303] font-medium mb-1">
              Suspended Vendors
            </Text>
            {isLoading ? (
        ...
        ListEmptyComponent={() =>
          isLoading && !refreshing ? (
            <View className="px-[21px] pt-4">
              {[1, 2, 3, 4, 5].map((i) => (
                <View key={i}>
                  <View className="p-[21px] flex-row items-center bg-white">
                    <View className="w-12 h-12 rounded-full bg-gray-100" />
                    <View className="flex-1 ml-4">
                      <View className="h-4 w-32 bg-gray-100 rounded mb-2" />
                      <View className="h-3 w-48 bg-gray-100 rounded" />
                    </View>
                  </View>
                  <View className="h-[11px] bg-[#F5F7FA]" />
                </View>
              ))}
            </View>
          ) : (
            ...
            <View>
              ...
              >
                <View className="w-12 h-12 rounded-full bg-blue-50 items-center justify-center">
                  <Text className="text-system-blue-light font-bold text-[16px]">
                    {initials}
                  </Text>
                </View>
                ...
                <View className="flex-1 ml-4 min-w-0">
                  ...
                  <View className="flex-row items-center mt-1 gap-2">
                    <View
                      className={`px-2 py-0.5 rounded-full ${item.is_active ? "bg-green-100" : "bg-red-100"}`}
                    >
                      <Text
                        className={`text-[10px] font-bold ${item.is_active ? "text-green-700" : "text-red-700"}`}
                      >
