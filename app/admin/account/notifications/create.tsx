...
export default function CreateNotificationScreen() {
  ...
  const [recipient, setRecipient] = useState<"Customers" | "Vendors" | "All">(
    "Customers",
  );
  ...
  const handleDateChange = (event: any, selectedDate?: Date) => {
    ...
    // iOS spinner fires continuously while scrolling — just track the value,
    // don't close or advance until the user taps Done.
    if (selectedDate) {
      setScheduledDate(selectedDate);
    }
  };
  ...
  const handleTimeChange = (event: any, selectedTime?: Date) => {
    ...
    // iOS spinner fires continuously while scrolling — just track the value,
    // don't close/finalize until the user taps Done.
    if (selectedTime && scheduledDate) {
      const finalDate = new Date(scheduledDate);
      finalDate.setHours(selectedTime.getHours());
      finalDate.setMinutes(selectedTime.getMinutes());
  ...
  const handleSave = async (isDraft: boolean) => {
    ...
    try {
      const body = {
        ...
        recipient_type: (recipient === "All"
          ? "ALL"
          : recipient === "Customers"
            ? "USERS"
            : "VENDORS") as any,
        recipient_group: (recipient === "All"
          ? "all"
          : recipient === "Customers"
            ? "customer"
            : "vendor") as any,
    ...
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      >
        ...
        <View className="mb-6">
          ...
          <View className="flex-row gap-2">
            {(["Customers", "Vendors", "All"] as const).map((r) => (
              ...
              >
                <Text
                  className={`text-[13px] font-bold ${recipient === r ? "text-system-blue-light" : "text-gray-500"}`}
                >
                  {r}
                </Text>
              </TouchableOpacity>
        ...
        {scheduledDate && (
          ...
          >
            <Text className="text-system-red text-[13px] font-medium">
              Clear Schedule
            </Text>
          </TouchableOpacity>
        )}
        ...
        <View className="gap-3">
          <Button onPress={() => handleSave(false)} isLoading={isLoading}>
          ...
          <Button
            ...
            isLoading={isLoading}
          >
            <Text className="text-system-blue-dark font-bold">
              Save as Draft
            </Text>
          </Button>
        </View>
            ...
            <View className="flex-row flex-wrap justify-between">
              {scheduleOptions.map((option) => (
                ...
                >
                  ...
                  <Text className="text-[13px] font-bold text-system-blue-dark text-center">
                    {option.label}
                  </Text>
                  {option.timeLabel && (
