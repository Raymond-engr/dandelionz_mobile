...
export default function RefundDisputesScreen() {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Toast.show({
                type: "error",
                text1: apiError(err, "Failed to approve refund."),
              });
            }
          },
        },
  ...
  const handleReject = async (refundId: number) => {
    if (!rejectionReason.trim()) {
      Toast.show({
        type: "error",
        text1: "Please provide a rejection reason.",
      });
      return;
    }
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Refund rejected. Customer notified.",
      });
      ...
      Toast.show({
        type: "error",
        text1: apiError(err, "Failed to reject refund."),
      });
    }
  };
  ...
  const renderItem = ({ item }: { item: RefundRequest }) => {
    ...
    return (
      <View className="mx-[21px] mb-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        ...
        {item.reason ? (
          <Text className="text-[13px] text-gray-500 mb-3 italic">
            "{item.reason}"
          </Text>
        ) : null}
