...
export default function UserDetail() {
  ...
  const [reviewRefundFlag, { isLoading: isReviewing }] =
    useReviewRefundFlagMutation();
        ...
        >
          ...
          <View className="flex-row items-center p-[21px]">
            ...
            <View className="ml-5 flex-1">
              ...
              <View className="flex-row gap-2">
                <View
                  className={`px-3 py-1 rounded-full ${user.status === "ACTIVE" ? "bg-[#dcfce7]" : "bg-[#fee2e2]"}`}
                >
                  <Text
                    className={`text-[12px] font-bold ${user.status === "ACTIVE" ? "text-[#16a34a]" : "text-[#dc2626]"}`}
                  >
          ...
          <View className="p-[21px] flex-row gap-4">
            <View className="flex-1 bg-[rgba(77,255,151,0.1)] p-4 rounded-xl border border-[rgba(77,255,151,0.2)]">
              <Text className="text-[12px] text-[#207d47] font-bold uppercase mb-1">
                Total Spend
              </Text>
            ...
            <View className="flex-1 bg-[rgba(3,4,130,0.1)] p-4 rounded-xl border border-[rgba(3,4,130,0.2)]">
              <Text className="text-[12px] text-[#030482] font-bold uppercase mb-1">
                Total Orders
              </Text>
              <Text className="text-[20px] font-bold text-system-blue-dark">
                {user.total_orders || "0"}
          ...
          {isCustomer && (
            <View className="p-[21px]">
              ...
              <Text className="text-[13px] text-[#00001180] mb-5">
                Review signal only — this never blocks or restricts the
                customer.
              </Text>
                  ...
                  {refund.needs_review ? (
                    <>
                      <Text className="text-[13px] text-[#6B7280] leading-5 mt-4">
                        ...
                        {(refund.thresholds.rate * 100).toFixed(0)}%). Review
                        their orders and mark reviewed once you have looked into
                        it.
                      </Text>
                      <TouchableOpacity
          ...
          {user.suspension_history && user.suspension_history.length > 0 && (
            <>
              ...
              <View className="p-[21px]">
                ...
                {user.suspension_history
                  .map((item: any) => (
                    <View
                      key={item.id}
                      className="bg-[#F5F7FA] p-4 rounded-xl border border-gray-50 mb-4"
                    >
                      <View className="flex-row justify-between items-center mb-2">
                        <View
                          className={`px-2 py-0.5 rounded ${item.action === "SUSPEND" ? "bg-red-100" : "bg-green-100"}`}
                        >
                          <Text
                            className={`text-[10px] font-bold ${item.action === "SUSPEND" ? "text-red-700" : "text-green-700"}`}
                          >
                            {item.action}
                          </Text>
                        </View>
                      ...
                      <Text className="text-[14px] text-system-blue-dark leading-5 mb-2 italic">
                        &quot;{item.reason}&quot;
                      </Text>
