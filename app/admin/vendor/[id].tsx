...
export default function VendorDetail() {
  ...
  const [suspendVendor, { isLoading: isSuspending }] =
    useSuspendVendorMutation();
  const [setVendorCommission, { isLoading: isSavingCommission }] =
    useSetVendorCommissionMutation();
  ...
  useEffect(() => {
    if (vendor?.commission_rate != null && vendor.commission_rate !== "") {
      setCommissionInput(
        String(
          Math.round(parseFloat(vendor.commission_rate) * 100 * 100) / 100,
        ),
      );
    } else {
      setCommissionInput("");
  ...
  const handleSaveCommission = async (clear = false) => {
    ...
    if (!clear && commissionInput.trim() !== "") {
      ...
      if (isNaN(pct) || pct < 0 || pct > 10) {
        Toast.show({
          type: "error",
          text1: "Enter a commission between 0 and 10%.",
        });
        return;
      }
    ...
    try {
      const res = await setVendorCommission({
        uuid: id!,
      ...
      Toast.show({
        type: "success",
        text1:
          value === null ? "Reset to platform default" : "Commission updated",
      ...
      Toast.show({
        type: "error",
        text1: "Error",
        text2: apiError(err, "Could not update commission."),
      });
    }
  };
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (action === "Approve Vendor") {
        ...
        Toast.show({
          type: "success",
          text1: "Vendor KYC verified successfully",
        });
      }
      refetch();
    } catch (err: any) {
        ...
        >
          ...
          <View className="flex-row items-center p-[21px]">
            ...
            <View className="ml-5 flex-1">
              ...
              <Text className="text-[14px] text-[#00001180] mb-3">
                {vendor.email}
              </Text>
              <View className="flex-row gap-2">
                <View
                  className={`px-3 py-1 rounded-full ${vendor.is_active ? "bg-[#dcfce7]" : "bg-[#fee2e2]"}`}
                >
                  <Text
                    className={`text-[12px] font-bold ${vendor.is_active ? "text-[#16a34a]" : "text-[#dc2626]"}`}
                  >
                    {vendor.is_active ? "Active" : "Suspended"}
                  </Text>
                </View>
                <View
                  className={`px-3 py-1 rounded-full ${vendor.is_verified_vendor ? "bg-[#dbeafe]" : "bg-[#fef9c3]"}`}
                >
                  <Text
                    className={`text-[12px] font-bold ${vendor.is_verified_vendor ? "text-[#1d4ed8]" : "text-[#a16207]"}`}
                  >
          ...
          <View className="p-[21px]">
            <Text className="text-[18px] font-bold text-system-blue-dark mb-5">
              Business Information
            </Text>
          ...
          <View className="p-[21px]">
            ...
            {vendor.recipient_code && (
              <InfoField label="Recipient Code" value={vendor.recipient_code} />
            )}
          </View>
          ...
          {/* Commission */}
          <View className="p-[21px]">
            <Text className="text-[18px] font-bold text-system-blue-dark mb-2">
              Commission
            </Text>
            <Text className="text-[13px] text-[#00001180] mb-4">
              ...
              . A per-product override, where set, takes precedence. Maximum
              10%.
            </Text>
            ...
            {vendor.commission_rate != null &&
              vendor.commission_rate !== "" && (
                <TouchableOpacity
                  onPress={() => handleSaveCommission(true)}
                  disabled={isSavingCommission}
                  className="mt-3"
                >
            ...
            <View className="p-[21px]">
              ...
              <View className="flex-row flex-wrap gap-2 mb-6">
                {availableActions.map((act) => (
                  ...
                  >
                    <Text
                      className={`text-[13px] font-bold ${action === act ? "text-system-blue-light" : "text-[#00001180]"}`}
                    >
                      {act}
                    </Text>
                  </TouchableOpacity>
