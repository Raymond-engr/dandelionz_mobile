...
const FREQUENCIES = [
  ...
  { id: "installment", label: "Installment Plan", sub: "Spread cost over several months" },
];
          ...
          {FREQUENCIES.map((f) => (
            <Pressable
              ...
              className={`p-5 rounded-[12px] border-2 flex-row items-center justify-between ${
                selected === f.id ? "border-system-blue-light bg-blue-50/30" : "border-gray-100 bg-[#F9FAFB]"
              }`}
            >
              <View className="flex-1">
                <Text className={`text-[16px] font-bold ${selected === f.id ? "text-system-blue-light" : "text-system-blue-dark"}`}>
                  {f.label}
                </Text>
                <Text className="text-[13px] text-[#6B7280] mt-1">{f.sub}</Text>
              </View>
              <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${selected === f.id ? "border-system-blue-light" : "border-gray-300"}`}>
                {selected === f.id && <View className="w-3 h-3 rounded-full bg-system-blue-light" />}
              </View>
            </Pressable>
          ))}
        </View>
