...
const DURATION_OPTIONS: {
...
}[] = [
  {
    value: "1_month",
    label: "1 Month Plan",
    description: "4 weekly payments over 1 month",
  },
  {
    value: "3_months",
    label: "3 Months Plan",
  ...
  },
  {
    value: "8_months",
    label: "8 Months Plan",
    description: "Split payment over 8 months",
  },
];
...
export default function CheckoutInstallments() {
  ...
  const [initializeInstallment, { isLoading }] =
    useInitializeInstallmentCheckoutMutation();
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (result.data?.authorization_url) {
        ...
        Toast.show({
          type: "error",
          text1: "Could not initialize installment plan. Please try again.",
        });
      }
    } catch (err: any) {
      Toast.show({
      ...
      <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
        ...
        <ScrollView className="flex-1 px-[21px]">
            ...
            {DURATION_OPTIONS.map((opt) => (
              ...
              >
                <View className="flex-1">
                  <Text
                    className={`text-[16px] font-bold ${duration === opt.value ? "text-system-blue-light" : "text-system-blue-dark"}`}
                  >
                    {opt.label}
                  </Text>
                ...
                <View
                  className={`w-6 h-6 rounded-full border-2 items-center justify-center ${duration === opt.value ? "border-system-blue-light" : "border-gray-300"}`}
                >
                  {duration === opt.value && (
          ...
          <Text className="text-[13px] text-[#6B7280] mb-4">
            ...
            {cartTotal > 0
              ? ` — order total is ${formatCurrency(cartTotal)}.`
              : "."}
          </Text>
          <View className="bg-blue-50 border border-blue-100 rounded-xl p-4">
            ...
            <View className="flex-row items-start mt-3">
              ...
              <Text className="text-[11px] text-[#6B7280] ml-1.5 flex-1">
                This first payment is by card via Paystack. Once your plan
                starts, you can pay the rest from your wallet or by card,
        ...
        <View
          className="px-[21px] pt-[21px]"
          style={{ paddingBottom: insets.bottom + 16 }}
        >
          <Button onPress={handleNext} isLoading={isLoading}>
            Proceed to Payment
          </Button>
        </View>
      </View>
