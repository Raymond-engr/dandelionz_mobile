...
export default function ProductDetailScreen() {
  ...
  // Reviews
  const { data: reviews, isLoading: isLoadingReviews, refetch: refetchReviews } = useGetProductReviewsQuery(slug);
  const [addProductReview, { isLoading: isSubmittingReview }] = useAddProductReviewMutation();

  // Wishlist
  const { data: wishlistItems = [] } = useGetWishlistQuery(undefined, { skip: !isAuthenticated });
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <Text className="text-lg font-semibold text-gray-900">Product Description</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-3">
            {images.map((img, idx) => (
              <TouchableOpacity
                ...
                className={`w-20 h-20 bg-gray-50 rounded-[12px] overflow-hidden border-2 ${selectedImage === idx ? 'border-system-blue-light' : 'border-transparent'}`}
              >
        ...
        {/* Info */}
        <View className="px-4">
          <Text className="text-xl font-semibold text-gray-900 mb-2">{product?.name}</Text>
          ...
          {Object.keys(variantOptions).length > 0 && (
            <View className="mb-6 space-y-4">
              {Object.entries(variantOptions).map(([category, valueSet]) => (
                <View key={category}>
                  <Text className="text-base font-semibold text-gray-900 mb-2 capitalize">Select {category}</Text>
                  <View className="flex-row flex-wrap gap-2">
                    {Array.from(valueSet).map((value) => (
                      <TouchableOpacity
                        key={value}
                        onPress={() => handleVariantSelect(category, value)}
                        className={`px-4 py-2 border rounded-[8px] ${selectedVariants[category] === value ? 'border-system-blue-light bg-blue-50' : 'border-gray-200'}`}
                      >
                        <Text className={`text-sm ${selectedVariants[category] === value ? 'text-system-blue-light font-semibold' : 'text-gray-600'}`}>
                          {value}
                        </Text>
                      </TouchableOpacity>
                    ))}
          ...
          <Text className="text-base text-gray-500 leading-6 mb-4">{product?.description}</Text>
          {product?.store_name && (
            <Text className="text-base font-medium text-system-blue-light mb-6">Store: {product.store_name}</Text>
          )}
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View>
              ...
              <Text className="text-[28px] font-bold text-system-blue-light">
                ₦{(parseFloat(product?.price || "0") * (1 - (product?.discount ?? 0) / 100)).toLocaleString()}
              </Text>
              {(product?.discount ?? 0) > 0 && (
                <View className="flex-row items-center gap-2 mt-1">
                  <Text className="text-sm text-gray-400 line-through">₦{parseFloat(product?.price || "0").toLocaleString()}</Text>
                  <View className="bg-red-50 px-2 py-0.5 rounded">
                    <Text className="text-[10px] font-bold text-red-600">-{product?.discount}%</Text>
                  </View>
                </View>
              )}
            </View>
            <View className="flex-row items-center bg-yellow-50 px-3 py-1 rounded-full">
              ...
              <Text className="text-sm font-semibold text-gray-900 ml-1">{product?.rating || "0.0"}</Text>
            </View>
          </View>
          ...
          <View className="flex-row gap-3 mb-8">
            <TouchableOpacity
              ...
              className={`w-12 h-12 border rounded-[12px] items-center justify-center ${isInWishlist ? 'border-red-100 bg-red-50' : 'border-gray-200'}`}
            >
              <Ionicons name={isInWishlist ? "heart" : "heart-outline"} size={24} color={isInWishlist ? "#ef4444" : "#666"} />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={handleToggleCart}
              disabled={isAddingToCart || isRemovingFromCart || !product?.in_stock}
              className={`flex-1 h-12 rounded-[12px] flex-row items-center justify-center gap-2 ${isInCart ? 'bg-red-50' : 'bg-system-blue-light'}`}
            >
              <Ionicons name="cart-outline" size={20} color={isInCart ? "#ef4444" : "#fff"} />
              <Text className={`font-semibold ${isInCart ? 'text-red-500' : 'text-white'}`}>
          ...
          <View className="border-t border-gray-100 pt-6">
            <Text className="text-xl font-semibold text-gray-900 mb-4">Reviews ({reviews?.length || 0})</Text>
            ...
            {isAuthenticated ? (
              <View className="bg-gray-50 p-4 rounded-[12px] mb-6">
                ...
                <View className="flex-row gap-2 mb-3">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <TouchableOpacity key={s} onPress={() => setUserRating(s)}>
                      <Ionicons name={s <= userRating ? "star" : "star-outline"} size={24} color={s <= userRating ? "#fbbf24" : "#d1d5db"} />
                    </TouchableOpacity>
                  ))}
                </View>
                <TextInput
                  ...
                  className="bg-white border border-gray-200 rounded-[8px] p-3 mb-3"
                  value={userComment}
                ...
                >
                  <Text className="text-white font-medium">{isSubmittingReview ? "Submitting..." : "Submit Review"}</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View className="bg-gray-50 p-4 rounded-[12px] mb-6 items-center">
                <Text className="text-gray-500 mb-2">Please sign in to write a review</Text>
                <TouchableOpacity onPress={() => router.push("/(auth)/login")}>
                  <Text className="text-system-blue-light font-semibold">Sign In</Text>
                </TouchableOpacity>
              </View>
            )}
                ...
                {reviews?.map((r: any) => (
                  <View key={r.id} className="border-b border-gray-100 pb-4">
                    <View className="flex-row justify-between mb-1">
                      <Text className="font-medium">{r.customer_name || "Anonymous"}</Text>
                      <Text className="text-[10px] text-gray-400">{new Date(r.created_at).toLocaleDateString()}</Text>
                    </View>
                    <View className="flex-row mb-2">
                      {[1,2,3,4,5].map(s => (
                        <Ionicons key={s} name="star" size={12} color={s <= r.rating ? "#fbbf24" : "#d1d5db"} />
                      ))}
                    </View>
