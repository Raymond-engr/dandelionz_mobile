...
import { isSystemNotification, resolveNotificationUrl } from "@/lib/utils";
...
export default function VendorNotificationsScreen() {
  ...
  // which is why this screen used to fall back through three different
  ...
  const renderHeader = () => {
    const hasUnread = filtered.some((n: any) => !n.is_read);
  ...
  return (
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      <View className="flex-row px-[21px] py-4 gap-4">
        ...
        >
          <Text
            className={`text-[14px] font-semibold ${filter === "all" ? "text-white" : "text-[#6B7280]"}`}
          >
            All
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text
            className={`text-[14px] font-semibold ${filter === "unread" ? "text-white" : "text-[#6B7280]"}`}
          >
            Unread
          </Text>
        </TouchableOpacity>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View>
            ...
            >
              <View
                className={`w-10 h-10 rounded-full items-center justify-center mr-4 ${item.is_read ? "bg-gray-100" : "bg-blue-100"}`}
              >
                <MaterialIcons
              ...
              <View className="flex-1">
                <View className="flex-row justify-between items-start mb-1">
                  <Text
                    className={`text-[15px] flex-1 pr-2 ${item.is_read ? "text-gray-600" : "font-bold text-system-blue-dark"}`}
                  >
                    {item.title}
                  </Text>
                ...
                <Text
                  className={`text-[13px] leading-5 ${item.is_read ? "text-gray-500" : "text-gray-700"}`}
                >
                  {item.message}
                </Text>
              </View>
