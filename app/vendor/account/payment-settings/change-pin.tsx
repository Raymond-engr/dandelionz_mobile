...
export default function VendorChangePin() {
  const router = useRouter();
  const insets = useSafeAreaInsets();

  const [setPin, { isLoading }] = useSetPaymentPINMutation();
  ...
  const handleUpdate = async () => {
    ...
    if (newP !== confP) {
      Toast.show({
        type: "error",
        text1: "New PIN and confirm PIN do not match.",
      });
      return;
    }
    ...
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Payment PIN updated successfully.",
      });
      router.back();
    } catch (err: any) {
      Toast.show({
